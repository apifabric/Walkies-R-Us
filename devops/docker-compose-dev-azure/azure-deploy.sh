...
resourcename="walkies-r-us"  # lower case, only
resourcegroup="walkies-r-us_rg"
...
# git clone https://github.com/apilogicserver/walkies-r-us.git
# cd walkies-r-us
...
az group create --name walkies-r-us_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group walkies-r-us_rg --sku S1 --is-linux
az container create --resource-group walkies-r-us_rg --name walkies-r-us --image apilogicserver/walkies-r-us:latest --dns-name-label walkies-r-us --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//walkies-r-us.westus.azurecontainer.io:5656'
